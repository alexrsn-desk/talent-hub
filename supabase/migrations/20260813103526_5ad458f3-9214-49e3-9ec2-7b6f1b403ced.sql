DROP TABLE IF EXISTS public.portal_api_keys CASCADE;
DROP TABLE IF EXISTS public.portal_interview_bookings CASCADE;
DROP TABLE IF EXISTS public.portal_stage_history CASCADE;
DROP TABLE IF EXISTS public.portal_feedback CASCADE;
DROP TABLE IF EXISTS public.portal_job_notes CASCADE;
DROP TABLE IF EXISTS public.portal_candidate_stage_overrides CASCADE;
DROP TABLE IF EXISTS public.portal_job_stage_content CASCADE;
DROP TABLE IF EXISTS public.portal_candidate_portals CASCADE;
DROP TABLE IF EXISTS public.portal_client_portals CASCADE;
DROP TABLE IF EXISTS public.portal_candidates CASCADE;
DROP TABLE IF EXISTS public.portal_jobs CASCADE;
DROP TABLE IF EXISTS public.portal_settings CASCADE;

ALTER TABLE public.jobs DROP COLUMN IF EXISTS portal_job_id;

DROP FUNCTION IF EXISTS public.portal_generate_token() CASCADE;
DROP FUNCTION IF EXISTS public.jobs_sync_portal_spec() CASCADE;
DROP FUNCTION IF EXISTS public.portal_seed_default_stage_content() CASCADE;
...
BEGIN
  ...
  DELETE FROM public.activity_events WHERE owner_user_id = v_user;
  DELETE FROM public.activity_log WHERE owner_user_id = v_user;
  DELETE FROM public.notifications WHERE owner_user_id = v_user;
  DELETE FROM public.import_history WHERE owner_user_id = v_user;
  DELETE FROM public.compliance_log WHERE owner_user_id = v_user;
  DELETE FROM public.compliance_audits WHERE owner_user_id = v_user;
